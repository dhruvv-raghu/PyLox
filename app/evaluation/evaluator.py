...
class NativeClock(LoxCallable):
    ...
    def call(self, interpreter, arguments: list): return time.time()
...
class Evaluator(Visitor, StmtVisitor):
    ...
    def visit_binary(self, node: Binary):
        ...
        # ... (same binary logic as before)
        if op_type == 'MINUS':
            self._check_number_operands(node.operator, left, right)
            return float(left) - float(right)
        if op_type == 'SLASH':
            self._check_number_operands(node.operator, left, right)
            if float(right) == 0.0: raise RuntimeError(f"Error: Division by zero.\n [line {node.operator.line}]")
            return float(left) / float(right)
        if op_type == 'STAR':
            self._check_number_operands(node.operator, left, right)
            return float(left) * float(right)
    ...
    def _check_number_operand(self, operator, operand):
        ...
        raise RuntimeError(f"[{operator.line}] Error: Operand must be a number.")

    def _check_number_operands(self, operator, left, right):
        if isinstance(left, float) and isinstance(right, float): return
        raise RuntimeError(f"[{operator.line}] Error: Operands must be numbers.")
